    ...
    for (const id of ids) {
        ...
        while (true) {
            ...
            try {
                ...
                const requ = ModbusRequest.createReadHoldRegister(1, r.id + 1, quantity);
                ...
                let value: number;
                switch (r.size) {
                    case 'u8':  value = resp.u8At(3) / r.factor; break;
                    case 's8':  value = resp.s8At(3) / r.factor; break;
                    case 'u16': value = resp.u16At(3) / r.factor; break;
                    case 's16': value = resp.s16At(3) / r.factor; break;
                    case 'u32': value = resp.u32At(3) / r.factor; break;
                    case 's32': value = resp.s32At(3) / r.factor; break;
                }
...
async function setDeggreeMinutes (value: number) {
    try {
        ...
        const requ = ModbusRequest.createWriteMultipleHoldRegisters(1, r.id + 1, 1,
...
async function getDeggreeMinutes (): Promise<number> {
    try {
        const r = regTab[43005];
        const requ = ModbusRequest.createReadHoldRegister(1, r.id + 1, 1);
        const resp = await modbus.write(requ.request);
        // debug.info('request: %s  -> response: %s', requ.request.frame, resp.frame);
        return resp.s16At(3) / r.factor;
    } catch (err) {
        debug.warn(err);
        return Number.NaN;
    }
...
async function setCompressorCutoffFrequency1 (start: number, stop: number) {
    try {
        let r = regTab[48662];
        let requ = ModbusRequest.createWriteMultipleHoldRegisters(1, r.id + 1, 1,
            [ start >= 0 ? start * r.factor : 65536 + start * r.factor]
        );
        let resp = await modbus.write(requ.request);
        debug.info('request: %s  -> response: %s', requ.request.frame, resp.frame);
        r = regTab[48664];
        requ = ModbusRequest.createWriteMultipleHoldRegisters(1, r.id + 1, 1,
            [ stop >= 0 ? stop * r.factor : 65536 + stop * r.factor]
        );
        resp = await modbus.write(requ.request);
        debug.info('request: %s  -> response: %s', requ.request.frame, resp.frame);
    } catch (err) {
        debug.warn(err);
    }
...
async function getCompressorMinFrequency (): Promise<number> {
    try {
        ...
        const resp = await modbus.write(requ.request);
        // debug.info('request: %s  -> response: %s', requ.request.frame, resp.frame);
        return resp.valueAt(3, r.size) / r.factor;
    } catch (err) {
        debug.warn(err);
        return Number.NaN;
    }
